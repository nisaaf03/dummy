...
"NGECOPY DARI 'ENV_1JOINT_NTARGET_V3' BEDANYA: "
" TARGET DIBIKIN BANYAK DAN DIACAK NAMUN CUMA 2 TITIK THETA TARGET "
" VARIASI TITIK START DIDIKITIN "
...
class ExcaRobo(gym.Env):
    def __init__(self, sim_active):
        ...
        self.joints_targets = np.array([[-0.294], #,1.4,0], # posisi joint awal kah? buat target reset
                                        [-0.144], #,0.59,1.47],
                                        [-0.257]]) #,1.46,0.276]])
        ...
        self.orientation_targets =  np.array([-0.7, 
                                              ...
                                              1.05 #0.74, #4
                                              -1.292])
        # self.joint1target = np.array([-0.773,-0.344, -0.475, 0.2, -0.5, -0.8]) # n titik target
        # self.joint1target = [0.2, -0.7, -0.9, 0.15, -0.6] #-0.773, 0.2, -0.9,
        self.joint1target = [ -0.7, -0.5]
        ...
        self.j = 0 # bener disini gak yaaa?
        self.orientation1joint = np.array([random.choice(self.joint1target)])
        # self.orientation1joint = self.get_target(self.j)
        # self.target_prev = self.orientation1joint
    ...
    def step(self, action):
        ...
        err_theta = self.orientation1joint - self.theta_now # compute error to determine the target
        if abs(err_theta) <= 0.05: 
            done = True
            # self.theta_target = random.choice(self.joint1target)
            self.orientation1joint = np.array([random.choice(self.joint1target)])
        elif np.any(self.theta_now > np.array(self.max_theta)) or np.any(self.theta_now < np.array(self.min_theta)): # or abs(error_theta) <= 0.05 :
            done = True
            # punishment = -1000
            punishment = 0
            self.reward = 0+punishment #harusnya reward + punishment, tp reward lgsg aku 0 in
        else:
    ...
    def reset(self):
        ...
        start_position = self.joints_targets[idx_start] if idx_start<=2 else self.joints_targets[0]
        # try:
    ...
    # def get_target(self,j):
    #     if j == 0:
    #         orientation1joint = np.array([self.joint1target[0]])
    #     elif j == 1:
    #         orientation1joint = np.array([self.joint1target[1]])
    #     elif j == 2:
    #         orientation1joint = np.array([self.joint1target[2]])
    #     elif j == 3:
    #         orientation1joint = np.array([self.joint1target[3]])
    #     elif j == 4:
    #         orientation1joint = np.array([self.joint1target[4]])
    #     else:
    #         orientation1joint = np.array([self.joint1target[5]]) # 5 titik target dulu
    #     return orientation1joint
